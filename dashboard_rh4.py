...
def generate_work_certificate(employee_data, template_path="attestation de travail.docx"):
    """
    Génère une attestation de travail personnalisée à partir d'un template Word
    """
    ...
    try:
        ...
        if not os.path.exists(template_path):
            # Créer un template basique si il n'existe pas
            doc = Document()
            
            # Ajouter le header
            header = doc.sections[0].header
            header_para = header.paragraphs[0]
            header_para.text = "ENTREPRISE XYZ - RESSOURCES HUMAINES"
            
            # Titre principal
            title = doc.add_heading('ATTESTATION DE TRAVAIL', 0)
            ...
            doc.add_paragraph()
            doc.add_paragraph("ATTESTE PAR LA PRÉSENTE que :")
            doc.add_paragraph()
            doc.add_paragraph("Monsieur/Madame : [NOM_COMPLET]")
            doc.add_paragraph("Né(e) le : [DATE_NAISSANCE]")
            doc.add_paragraph("À : [LIEU_NAISSANCE]")
            doc.add_paragraph()
            doc.add_paragraph("A été employé(e) dans notre entreprise en qualité de [POSTE] depuis le [DATE_ENTREE].")
            doc.add_paragraph()
            ...
            doc.add_paragraph()
            doc.add_paragraph("Le Directeur des Ressources Humaines")
            doc.add_paragraph()
        ...
        replacements = {
            ...
            '[DATE_NAISSANCE]': employee_data.get('Date de naissance', '').strftime('%d/%m/%Y') if pd.notna(employee_data.get('Date de naissance')) else 'Non renseigné',
            '[LIEU_NAISSANCE]': employee_data.get('Lieu de naissance', 'Non renseigné'),
            '[POSTE]': employee_data.get('Poste', 'Non renseigné'),
            '[DATE_ENTREE]': employee_data.get('DateEntree', '').strftime('%d/%m/%Y') if pd.notna(employee_data.get('DateEntree')) else 'Non renseigné',
            '[DATE_GENERATION]': datetime.now().strftime('%d/%m/%Y'),
            '[NOM_ENTREPRISE]': 'VOTRE ENTREPRISE',
            '[DIRECTEUR_RH]': 'M./Mme LE DIRECTEUR RH',
            '[VILLE]': 'VOTRE VILLE',
        ...
        # Remplacer dans les tableaux si présents
        ...
        # Remplacer dans les headers et footers
...
def main():
    ...
    if not DOCX_AVAILABLE:
        ...
        <div style='background: linear-gradient(135deg, #28a745 0%, #20c997 100%); 
                    padding: 15px; border-radius: 10px; margin-top: 20px;'>
            <h4 style='color: white; margin-bottom: 10px;'>📋 Attestations</h4>
            <p style='color: #f8f9fa; font-size: 0.9em; margin: 0;'>
                Générateur d'attestation disponible dans l'onglet principal !
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not DOCX_AVAILABLE:
        ...
        st.markdown("""
        <div class="info-box">
            <h4>📋 Génération Automatique d'Attestations</h4>
            <p>Sélectionnez un employé pour générer automatiquement son attestation de travail personnalisée.</p>
            <p><em>Le document sera basé sur le template "attestation de travail.docx" ou créé automatiquement.</em></p>
        </div>
        """, unsafe_allow_html=True)
                        ...
                        with col_btn2:
                            if st.button("📄 Générer l'Attestation de Travail", type="primary", use_container_width=True):
                                with st.spinner("Génération de l'attestation en cours..."):
                                    ...
                                    doc_buffer, error = generate_work_certificate(employee_row)
